...
export async function catalogoServicios(): Promise<ServicioCatalogo[]> {
  const rows = await consulta(
    `select clave, nombre, categoria, recurrente, precio_variable, precio_mes1, precio_resto,
            min_meses, aplica_cs, aplica_referido, aplica_reserva, por_persona, precio_persona, costo_persona, unidad
  ...
  return rows.map((r: Record<string, unknown>) => ({
    ...
    costoPersona: r.costo_persona != null ? Number(r.costo_persona) : null,
